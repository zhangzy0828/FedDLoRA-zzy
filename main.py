...
# CUDA_VISIBLE_DEVICES=2 python main.py 2>&1 | tee 

def fl_finetune(
        # model/data params
        global_model: str = '/2014110093/zhangzy/FedDPA/data/Llama2-7B',
        data_path: str = '/2014110093/zhangzy/FedDPA/data/FLAN1/train',
        output_dir: str = '/2014110093/zhangzy/FedDPA/output/train/KD+neft',
        ...
        resume_round: int = 0,              #Resume training from the breakpoint
    ...
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        print(
            ...
            f"kd_alpha: {kd_alpha}\n"
            f"neftune_noise_alpha: {neftune_noise_alpha}\n"
        )
    assert (
        global_model
    ...
    # 断点续训加载逻辑 ==============================================
    if resume_round > 0:
        print(f"RESUMING training from Round {resume_round}...")
        last_round_dir = os.path.join(output_dir, str(resume_round - 1))
        checkpoint_path = os.path.join(last_round_dir, "adapter_model.bin")
        
        if os.path.exists(checkpoint_path):
            print(f"Resume Loading global checkpoint from: {checkpoint_path}")
            # 加载权重
            adapters_weights = torch.load(checkpoint_path, map_location="cpu")
            # 将权重设置给 default adapter
            set_peft_model_state_dict(model, adapters_weights, "default")
    ...
    for epoch in tqdm(range(resume_round, num_communication_rounds)):
        ...
        for client_id in selected_clients_set:
            ...
            # =======================================================
            if epoch > 0:
                client.build_local_trainer(tokenizer, local_micro_batch_size, gradient_accumulation_steps,
                                       local_num_epochs, local_learning_rate, group_by_length, ddp,
                                       kd_alpha=kd_alpha,          # Phase 1 开启
                                       kd_temperature=kd_temperature)
                
                print("Initiating the LOCAL training of Client_{}".format(client_id))
                print(f"Initiating LOCAL training (KD enabled, alpha={kd_alpha})...")
                print("local train num = ",local_num_epochs)
                client.initiate_local_training(local=True, epoch=epoch, neftune_alpha=neftune_noise_alpha)

                client.set_local_dual_mode(epoch=epoch, prv=prv if client_selection_strategy=='subset' else None)
                
                print("Local training starts ... ")
                client.train()
                
                print("\nTerminating the LOCAL training of Client_{}".format(client_id))
                model, _, _, _ = client.terminate_local_training(epoch, local_dataset_len_dict, previously_selected_clients_set, config, local=True)
            ...
            print("Initiating the GLOBAL training of Client_{}".format(client_id))
            print("global train num = ",local_num_epochs)

            client.initiate_local_training(local=False, neftune_alpha=0)


            print("Global training starts ... ")
            client.train()

            print("\nTerminating the GLOBAL training of Client_{}".format(client_id))
            model, local_dataset_len_dict, previously_selected_clients_set, last_client_id = client.terminate_local_training(
                epoch, local_dataset_len_dict, previously_selected_clients_set, config, local=False)
        ...
        # Save Global Model
        new_adapter_weight = get_peft_model_state_dict(model, adapter_name='default')
        torch.save(new_adapter_weight, os.path.join(output_dir, str(epoch), "adapter_model.bin"))
        config.save_pretrained(output_dir)
        print('communication round: ', epoch )
        # Evaluation
...
previously_selected_clients_set,config,local=local_model)
